...
- **Scope/Applicability**: Required for all deliverables; scope scales by app type.
...
**This role is REQUIRED** for all apps. If the app is not deployable (e.g., library/CLI), define and document the release equivalent with Sponsor approval.
...
### Scope Scaling (Examples)
- CLI tools/libraries: focus on release packaging, distribution, install/upgrade steps, and rollback guidance.
- Prototypes/demos: define explicit release equivalent and document limitations.
- Production services: full deployment, monitoring, alerting, and runbooks.

**Decision**: PM and Architect define the operational scope during design, but the Operations role always participates.
