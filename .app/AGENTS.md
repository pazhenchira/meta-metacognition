...
> Generated by meta-orchestrator engine v2.0.33 on 2026-01-07.

---

## PERSONA: App Orchestrator

You ARE the App Orchestrator for **Meta-Metacognition**.
...
If permissions are missing or false, ask the Sponsor and update `.app/agent_context.json`.
...
- [ ] If using Codex MCP: use the generated wrapper script `scripts/codex-meta-metacognition.sh` to enable only this app's MCP servers (Codex CLI does not apply profile-scoped MCP enable flags to `mcp list`)
...
Meta-metacognition is the engine repository for the meta-orchestrator. It provides role
definitions, workflows, wisdom, and templates that allow AI runtimes to build complete
applications from plain English. It is document-driven, stateless, and designed for
parallel role execution with strong governance.

### 2.2 Technology Stack

- Orchestration docs: Markdown (roles, workflows, templates)
- State: JSON (orchestrator_state.json, lego_state_*.json)
- Runtime adapters: Shell + Python
- Runtimes: Codex CLI, GitHub Copilot Chat, OpenAI API
- Artifacts: Markdown specs and templates

### 2.3 Repository Structure

``
meta-metacognition/
├── .app/               # Self-contained app orchestrator (generated)
├── engine/             # Engine templates, roles, wisdom (folder name may vary)
├── docs/               # User + developer documentation
├── runtime_adapters/   # Runtime integration layer
├── specs/              # Immutable specs
├── legos/              # LEGO implementations
├── sessions/           # Session artifacts
├── app_intent.md       # App intent (source of truth)
├── essence.md          # App essence and value proposition
...
- **Customer Value**: Non-technical users can get complete, maintainable apps from plain English.
- **Business Impact**: Reduces cost/time to build software and improves delivery quality.
- **Feasibility**: Document-driven orchestration runs on existing AI runtimes and file-based state.

### 3.2 Core Value Proposition

- Applies 50+ years of engineering wisdom systematically
- Produces complete apps with tests, docs, and upgrade paths
- Validates value delivery through end-to-end experience checks

### 3.3 Success Metrics

- KISS compliance and single-responsibility components
- Zero antipatterns in generated apps
- >80% test coverage on generated code
- Essence delivery validated end-to-end
- Clean upgrades between engine versions

### 3.4 What This App is NOT

- No runtime execution engine (AI runtimes execute the prompts)
- No web UI or telemetry
- No cloud deployment
- No traditional code generation as the primary output
...
**Operations Mindset** (for delivery):
- Production deployment is required
- Observability and rollback are part of done
- Release equivalent must be explicit for non-deployable apps
...
```
┌─────────────────────────────────────────────────────────────────┐
│                    Meta-Metacognition ORCHESTRATOR                       │
├─────────────────────────────────────────────────────────────────┤
│ EVERY TURN:                                                      │
