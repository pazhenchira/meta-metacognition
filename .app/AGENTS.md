...
- Ensure each `[mcp_servers.meta-metacognition__<role>]` in `~/.codex/config.toml` sets `tool_timeout_sec` to `mcp_tool_timeout_seconds`
- Ensure each MCP server entry sets `cwd` to the app root for context isolation
